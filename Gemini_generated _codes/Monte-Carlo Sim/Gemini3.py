...
def run_max_sharpe_portfolio():
    # ==========================================
    # 1. SETUP & DATA DOWNLOAD
    # ==========================================
    print("--- Starting Max Sharpe Optimization ---")
    ...
    # ==========================================
    # 2. MAXIMIZE SHARPE RATIO (STOCKS ONLY)
    # ==========================================
    print("\nOptimizing Stocks for MAXIMUM SHARPE RATIO...")

    stock_ret = returns[stock_tickers]
    stock_cov = stock_ret.cov()
    mean_stock_ret = stock_ret.mean()

    # Risk Free Rate (Approx 3.5%)
    rf_rate = 0.035
    ...
    # Constraints: Sum = 1.0 (100% of Stock Bucket)
    constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})

    # Bounds:
    # Min 2% (0.02) to keep diversification
    # Max 40% (0.40) to prevent putting EVERYTHING into just one stock (common safety rule)
    ...
    # ==========================================
    # Scale stocks to 70% of total
    final_stock_w = opt_stock_sub_weights * 0.70
    ...
    print("\n" + "=" * 55)
    print(f"{'MAX SHARPE PORTFOLIO WEIGHTS':^55}")
    print("=" * 55)
    ...
    sharpe = (expected_cagr - rf_rate) / (port_std * np.sqrt(252))
