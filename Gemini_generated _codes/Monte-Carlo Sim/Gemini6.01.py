...
def run_final_super_sim():
    ...
    cons = [
        ...
        # Macro Guardrails
        {'type': 'ineq', 'fun': lambda x: np.sum(x[:13]) - 0.50},  # Stocks Min 50%
        {'type': 'ineq', 'fun': lambda x: 0.80 - np.sum(x[:13])},  # Stocks Max 80%
        {'type': 'ineq', 'fun': lambda x: np.sum(x[13:17]) - 0.15},  # Bonds Min 15%
        {'type': 'ineq', 'fun': lambda x: np.sum(x[17:]) - 0.05}  # Gold Min 5%
    ]

    # BOUNDS: Force 3% Min on ALL Stocks (Diversity)
    bounds = []
    for i in range(n_assets):
        if i < 15:  # The 15 Stocks
            bounds.append((0.03, 0.20))  # Min 3%, Max 20%
        else:  # Bonds & Gold
