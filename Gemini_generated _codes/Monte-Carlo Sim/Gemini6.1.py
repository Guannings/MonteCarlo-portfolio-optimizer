...
def run_final_super_sim():
    ...
    # 1. DEFINE THE 20 ASSETS
    # We will use the list: 9 Core + 6 New + 4 Bonds + 1 Gold

    core_stocks = ['GOOGL', 'JPM', 'PEP', 'UNH', 'ASML', 'LVMUY', 'NVS', 'TTE' , 'SIEGY']
    new_stocks = ['ACN', 'COST', 'MCD', 'BRK-B', 'SONY' , 'ITOCY']
    bonds = ['BGRN', 'LQD', 'TLT', 'IEF']
    gold = ['GLD']
    ...
    # 2. OPTIMIZATION (Target Vol 12% + Diversity)
    print("Optimizing: Target Volatility 12% with Broad Diversification...")
    ...
    cons = [
        ...
        {'type': 'ineq', 'fun': lambda x: 0.80 - np.sum(x[:n_stocks])},  # Stocks Max 80%
        {'type': 'ineq', 'fun': lambda x: np.sum(x[n_stocks : n_stocks+n_bonds]) - 0.15},  # Bonds Min 15%
        {'type': 'ineq', 'fun': lambda x: np.sum(x[-n_gold]) - 0.05}  # Gold Min 5%
    ]
    ...
    for i in range(n_assets):
        if i < n_stocks:
            ...
            bounds.append((0.03, 0.20))
        elif i < n_stocks + n_bonds:
            # It is a Bond: Allow 0% (but macro rule forces group total to 15%)
            bounds.append((0.00, 0.25))
        else:
            # It is Gold: Allow 0% (but macro rule forces group total to 5%)
            bounds.append((0.00, 0.25))
    bounds = tuple(bounds)
