...
def run_global_optimization():
    print("--- Starting Global 'Max Sharpe' Optimization ---")
    ...
    print("Finding the perfect Asset Allocation mix...")
    ...
    # 2. Flexible Asset Allocation Rules (The "Guardrails")
    # Indices: Stocks (0-8), Dom Bonds (9-10), Bond ETFs (11-12), Gold (13)

    # Rule A: Stocks between 50% and 75%
    cons.append({'type': 'ineq', 'fun': lambda x: np.sum(x[:9]) - 0.50})  # Min 50%
    cons.append({'type': 'ineq', 'fun': lambda x: 0.75 - np.sum(x[:9])})  # Max 75%

    # Rule B: Bonds at least 20%
    cons.append({'type': 'ineq', 'fun': lambda x: np.sum(x[9:13]) - 0.20})

    # Rule C: Gold between 5% and 15%
    cons.append({'type': 'ineq', 'fun': lambda x: np.sum(x[13:]) - 0.05})
    cons.append({'type': 'ineq', 'fun': lambda x: 0.15 - np.sum(x[13:])})

    # Bounds for individual assets (Don't let one asset be > 20%)
    bounds = tuple((0.00, 0.20) for _ in range(n_assets))

    res = minimize(get_negative_sharpe, init_guess, method='SLSQP', bounds=bounds, constraints=cons, tol=1e-10)
    final_weights = res.x

    # 3. PRINT WEIGHTS
    portfolio_weights = dict(zip(all_tickers, final_weights))
    print("\n" + "=" * 55)
    print(f"{'GLOBAL OPTIMIZED PORTFOLIO':^55}")
    print("=" * 55)
    ...
    for t in all_tickers:
        ...
        if w > 0:  # Only show significant holdings
            print(f"{t:<10} {a_type:<15} {w:>10.2%}      ${total_val * w:,.0f}")

    # Calculate Macro Allocation
    ...
    plt.title('Global Optimized Portfolio: 5-Year Outlook')
    ...
    plt.title('Return Distribution (Higher Efficiency)')
